...
export function serializeLocation(loc: Loc): string {
  const parts: string[] = [];
  if (loc.path.length) parts.push('p=' + loc.path.join('~'));
  if (loc.node) parts.push('n=' + loc.node);
  if (loc.persona) parts.push('persona=' + loc.persona);
  const v = Object.entries(loc.variants);
  if (v.length) parts.push('v=' + v.map(([base, sel]) => `${base}:${sel}`).join(','));
  return parts.join('&');
}
...
export function parseLocation(search: string): Loc {
  ...
  return {
    path: (q.get('p') ?? '').split('~').filter(Boolean),
    node: q.get('n') || null,
