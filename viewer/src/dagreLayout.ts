...
export function dagreLayout(boxes: SizedBox[], edges: Array<[string, string]>, vertical: boolean, g: LayoutGaps): Layout {
  ...
  boxes.forEach((n) => graph.setNode(n.id, { width: n.w, height: n.h }));
  // skip self-loops and any endpoint not in the box set.
  edges.forEach(([a, b]) => { if (ids.has(a) && ids.has(b) && a !== b) graph.setEdge(a, b); });

  dagre.layout(graph);

  const pos: Layout['pos'] = {};
  boxes.forEach((n) => {
    const nd = graph.node(n.id) as { x: number; y: number; width: number; height: number };
